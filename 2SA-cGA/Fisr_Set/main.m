% -- Synchronous Cellular Genetic Algorithm with static Narrow grid Using a Back Evolution of Pm and restart mechanism --------
% -- Information ---
%This code has been checked the 25/10/2017 at 12:43  and found all correct
clear all
...
global state_of_the_art 
       state_of_the_art = [98535  97156 95038 173701 182331  174519 307695 287149  264204  385927 357368  370868];
...
%% -- Restart phase --------
if it == round((3 * iter)/5)
    % --------- Create the new population ------------------
    population = [];
    fit = [];
    pop_restart =  repmat(gbest,(indiv-1),1);
    allowed_cata_bits = floor(catclysmic_rate * Dimension);
    for s=1:(indiv-1)
        cata_bits = randi([1,Dimension],1,allowed_cata_bits);
        pop_restart(s,cata_bits) = abs(pop_restart(s,cata_bits) - 1);
    end
    population = [gbest;pop_restart];
    % --------- Evaluate the new population ----------------
    for w=1:indiv  
        result  = RC_Function(population(w,:),Dimension,network,neighbourhoud);
        fit = [fit , result];
    end
    % --------- Extract the new best individual ------------
    [best_fit,indbest] = min(fit);
    gbest = population(indbest,:);
    % ----------- Increase the iterations because the stop and start contain an extra loop of evaluation --------------------------
    it = it + 1;
end
%% -- Record the difference between the best individual and the 2nd -------
if it == round(iter/2) 
    diffone = find(gbest == 1);
    % ----- write the gbest ------------
    name = strcat(instance,'_gbest_12.txt');
    fileID = fopen(name,'w');
    fprintf(fileID,'%d\n',diffone);
    fclose(fileID);   
    % ----- write the fitness value ----
    name = strcat(instance,'_fit_12.txt');
    fileID = fopen(name,'w');
    fprintf(fileID,'%d\n',transpose(fit));
    fclose(fileID);    
end
if it  == round((3 * iter)/4)
    difftwo =  find(gbest == 1);
    % ----- write the gbest ------------
    name = strcat(instance,'_gbest_34.txt');
    fileID = fopen(name,'w');
    fprintf(fileID,'%d\n',difftwo);
    fclose(fileID); 
    % ----- write the fitness value ----
    name = strcat(instance,'_fit_34.txt');
    fileID = fopen(name,'w');
    fprintf(fileID,'%d\n',transpose(fit));
    fclose(fileID); 
end
if it == iter
    diffthree =  find(gbest == 1); 
    % ----- write the gbest ------------
    name = strcat(instance,'_gbest_1.txt');
    fileID = fopen(name,'w');
    fprintf(fileID,'%d\n',diffthree);
    fclose(fileID);  
    % ----- write the fitness value ----
    name = strcat(instance,'_fit_1.txt');
    fileID = fopen(name,'w');
    fprintf(fileID,'%d\n',transpose(fit));
    fclose(fileID); 
end
%% -- Encreasing The Mutation Probability ---------------------------------
pm = (2 + ((Dimension-2)/(iter-1))*it)^(-1);
...
for i=0:(grid_y - 1)
    for j=1:grid_x
        ...
        % ----- In order to avoid having non feasible solutions ------------
        if isequal(child,zeros(1,Dimension)) == 1
            child(randi(Dimension)) = 1;
        end
...
xlwrite(name,transpose(mutation_evo),13);
end
