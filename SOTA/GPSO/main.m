...
%NB: according to the email and Moraglio's original work, the best social Gi is the best particle among all,
%and the historical best Hi is the the best state found so far for the particle itself.

%% NB: I reported the results of the GPSO and HNN-BD in our article and compared them to the ones of algorithms
% that have been run 30 executions, even if GPSO and HNN-BD have been run only 10 executions. The reason for that
% That I followed the work done in "Studying the Reporting Cells Strategy in a Realistic Mobile Environment"
% I fact, they compared the scatter search, differential evolution and NSGA II which are run 30 executions with 
% the GPSO and HNN-BD which are run 10 executions.

% Hiwever, this raises a small issue, if applying the condition in Pseudocode within paper (see above)
% the Xi = Hi = Gi. Therfore, the three parent based Crossover they becomes a one parent crossover.
...
for w=1:indiv
    ...
    if rand <= Pc
        vect_3PMBCX = randi([1,3],1,Dimension); % normally Wa, Wb and Wc = 0.33 but since they have equal probability toapear I prefered generating aa random number from a uniform distribution.
        best_indices = find(vect_3PMBCX == 3); % find the genees that corresponds to the global best
        historic_indices = find(vect_3PMBCX == 2); % find the genees that corresponds to the historical best ( best of the that particle itself)
        indiv_indices = find(vect_3PMBCX == 1); % find the genees that corresponds to solution itself
